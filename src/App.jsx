...
// ─── Simple Hash Router ───────────────────────────────────────
// بدون react-router-dom — يشتغل مباشرة
// الروابط:
//   /              → صفحة الهبوط
//   #/admin        → لوحة تحكم المدير
//   #/dashboard    → لوحة العميل
//   #/widget-demo  → معاينة الويدجت
...
export default function App() {
  ...
  // ─── Route Guard: Admin requires password ─────────────────
  ...
  const ADMIN_PASSWORD = "admin2026"; // غيّره قبل الرفع!
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          borderRadius: 24, padding: 48, width: 360, textAlign: "center",
        }}>
          ...
          <h2 style={{ color: "#fff", fontSize: 20, fontWeight: 800, marginBottom: 8 }}>لوحة تحكم AixraAI</h2>
          <p style={{ color: "#444", fontSize: 13, marginBottom: 32 }}>أدخل كلمة المرور للمتابعة</p>
          <input
            type="password"
            ...
            onKeyDown={e => {
              if (e.key === "Enter") {
                if (adminPass === ADMIN_PASSWORD) { sessionStorage.setItem("aixra_admin", "true"); setAdminAuth(true); }
          ...
          />
          {authError && <p style={{ color: "#EF4444", fontSize: 12, marginBottom: 12 }}>كلمة المرور غير صحيحة</p>}
          <button
            onClick={() => {
              if (adminPass === ADMIN_PASSWORD) { sessionStorage.setItem("aixra_admin", "true"); setAdminAuth(true); }
              else setAuthError(true);
          ...
          <button onClick={() => { window.location.hash = "/"; }} style={{ marginTop: 16, background: "transparent", border: "none", color: "#444", fontSize: 13, cursor: "pointer" }}>← العودة للموقع</button>
        </div>
      </div>
    );
  }
